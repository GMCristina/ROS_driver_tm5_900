// Node for obstacle avoidance with pick and place trajectory
...
void ObstacleDetectedCallback(ros::NodeHandle &nh, const tm_msgs::ObstacleDetected::ConstPtr& msg)
  {
    ...
    ROS_INFO_STREAM("PICK: First clear");
    ...
    //IMPORTANT: Wait for pick_place_trajectory_node to stop
    ROS_INFO_STREAM("PICK: Wait for pick_place_trajectory_node to stop");
    bool waiting = true;
    std::string node = "/pick_place_trajectory_node";
    while(waiting) {
      ros::V_string v;
      ...
      for (int i=0; i < v.size(); i++) {
            if (v[i].compare(node)==0) {
              ROS_INFO_STREAM("PICK: still exists " << v[i] <<"node... wait..." );
              waiting = true;
            }
      }
      ros::Duration(0.5).sleep();
    }
    ROS_INFO_STREAM("PICK: pick_place_trajectory_node terminated, send clear and home...");
    ...
    std::string cmd = "StopAndClearBuffer()\r\nPTP(\"JPP\",0,0,0,0,0,0,5,500,0,false)"; //stop+home
    ...
    {
      if (srv1.response.ok) ROS_INFO_STREAM("PICK: Sent script to robot");
      else ROS_WARN_STREAM("Sent script to robot , but response not yet ok ");
    }else{
        ROS_ERROR_STREAM("PICK: Error send script to robot");
        }
...
{
  //Node initialization
  ros::init(argc, argv, "obstacle_avoidance_naive_pick_node");
  ros::NodeHandle nh;
